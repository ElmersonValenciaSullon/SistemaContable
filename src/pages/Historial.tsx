...
import type { Transaction, Category, OpResult, FiltrosHistorial } from '../types';
...
// FIX: FiltrosHistorial.busqueda eliminado de los filtros locales
// La búsqueda viene del prop 'busqueda' (controlado desde App → Topbar)
// y NO se duplica en el estado local de filtros para evitar conflictos
const FILTROS_INIT: Omit<FiltrosHistorial, 'busqueda'> = {
  tipo: 'all',
  fechaDesde: '',
  fechaHasta: '',
  categoriaId: '',
...
type FiltrosLocales = Omit<FiltrosHistorial, 'busqueda'>;
...
export default function Historial({
  ...
  const [filtros, setFiltros] = useState<FiltrosLocales>(FILTROS_INIT);
  const [mostrarFiltros, setMostrarFiltros] = useState(false);

  const setF = <K extends keyof FiltrosLocales>(k: K, v: FiltrosLocales[K]) =>
    setFiltros((f) => ({ ...f, [k]: v }));

  // FIX: una única fuente de verdad para búsqueda — usa el prop, no un estado duplicado
  const filtradas = useMemo(
    () =>
      transactions.filter((tx) => {
        ...
        if (
          busqueda &&
          !tx.description.toLowerCase().includes(busqueda.toLowerCase()) &&
          !(tx.notes ?? '').toLowerCase().includes(busqueda.toLowerCase())
        )
          return false;
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-5 animate-fade-in">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className={`btn-ghost text-xs py-1.5 ${
              hayFiltros ? 'border-blue-300 text-blue-600 bg-blue-50' : ''
            }`}
          >
          ...
          <div
            className={`ml-auto text-sm font-extrabold ${
              totalFiltrado >= 0 ? 'text-emerald-600' : 'text-red-500'
            }`}
          >
            {totalFiltrado >= 0 ? '+' : ''}
      ...
      <div className="flex items-center justify-between px-1">
        <p className="text-xs text-slate-400 font-medium">
          ...
          {busqueda && (
            <span className="text-blue-500 ml-1">· "{busqueda}"</span>
          )}
        </p>
