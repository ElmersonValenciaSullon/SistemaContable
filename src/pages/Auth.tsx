...
export default function Auth() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (modo === 'signup') {
        ...
        // FIX: mejor manejo del error 400 de Supabase
  ...
  // ── FIX: Recuperación de contraseña — antes era un botón sin funcionalidad ──
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      setMensaje(
        `✓ Correo enviado a ${email}. Revisa tu bandeja de entrada y sigue el enlace para crear una nueva contraseña.`
      );
    } catch (err: any) {
      const msg: string = err.message || '';
  ...
  // ─────────────────────────────────────────────
  // RENDER: Pantalla de recuperar contraseña
  // ─────────────────────────────────────────────
  if (modo === 'reset') {
    return (
              ...
              {mensaje && (
                <button
                  ...
                  className="
                    ...
                    text-slate-700 font-bold py-3.5 rounded-xl text-sm
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {modo === 'signin' && (
                  // FIX: antes era un botón sin onClick — ahora navega a modo 'reset'
                  <button
                    type="button"
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="
                    ...
                    transition-colors p-0.5 rounded-lg hover:bg-slate-100
                  "
              ...
              {/* Indicador de fuerza (solo signup) */}
...
// Panel derecho extraído para evitar duplicar código
  ...
  return (
    ...
    `}>
      ...
      <div className="relative z-10 flex flex-col items-center gap-10 px-12">
        {/* Ícono central en lugar de imagen base64 pesada */}
        <div className="w-48 h-48 bg-white/30 backdrop-blur-sm rounded-3xl flex items-center justify-center shadow-2xl shadow-blue-300/30 animate-[float_4s_ease-in-out_infinite]">
          <ShieldCheck className="w-24 h-24 text-blue-600" strokeWidth={1.5} />
        </div>
